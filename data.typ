#let EMPTY_BODY = []

#let my_name = [Баймурадов Т. Р.]
#let my_group = [ИС/б-23-1-о]

// sem 5
#let ТПО = [Тестирование программного обеспечения]
#let ТСИС = [Технические средства информационных систем]
#let ВЕБ = [Веб-технологии]
#let БД = [Управление данными]
#let NET = [Платформа .Net]

// sem 6
#let ИКСС = [Инфокоммуникационные системы и сети]
#let ТОИ = [Технологии обработки информации]
#let СИСАНАЛ = [Методы системного анализа и проектирования информационных систем]
#let DataMining = [Интеллектуальный анализ данных]

#let Prof_DataMining = (
  [Сырых О. А.],
  [ст. преподаватель кафедры "Информационные технологии и системы"],
)
